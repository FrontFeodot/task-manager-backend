...
export const createColumn = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!title || !boardId) {
        res.status(500).send(new CustomResponse({isError: 1, message: 'Missing title'}))
    }
    ...
    try {
        ...
        if (isAlreadyExist) {
            res.status(409).send(new CustomResponse({isError: 1, message: 'Column already exist, please choose another name'}))
        }
        const newColumn = {title, order, columnId: nanoid()}
        boardResponse.set('columns', [...columns, newColumn])
        boardResponse.save();
        res.status(200).send(new CustomResponse({isSuccess: 1, message: 'Column created successfully'}))
        ...
        res.status(500).send(new CustomResponse({isError: 1, message: 'Something went wrong while creating column', payload: err}))
    }
} 
...
export const deleteColumn = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const removedColumn = findIndex(board.columns, (column) => column.columnId === columnId)
    ...
    if (tasksPath === 'delete') {
      ...
      const lastTaskOrderInColumn = await Task.findOne({userId, boardId, columnId: tasksPath}).sort({ order: -1 }).exec()
    ...
    await board.save()
    res.status(200).send(new CustomResponse({isSuccess: 1, message: 'Column deleted successfully'}));
  ...
  res.status(500).send(new CustomResponse({isError: 1, message: 'Something went wrong while deleting column', payload: err}))
